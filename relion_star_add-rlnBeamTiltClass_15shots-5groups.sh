...
###                                                                           ###
### Script to add rlnBeamTiltClass to a relion star file based on micrograph  ###
###     name                                                                  ###
###                                                                           ###
...
if [ -z $1 ] ; then
 ...
 echo '     Written for acquisition of a diamond hole matrix (5 holes) with 3 shots per hole; each hole will be the same BeamTiltClass instead of each shot; results in 5 beamtilt classes' 
 echo
 echo "Usage ${0##*/} (1)"
...
awk -v X=${IMGCOL} '{print $X}' old_star_noheader.tmp | sed -e 's/.mrcs//g' -e 's/_/ /g' | awk '{printf "%i\n", $NF}' | sed -e 's/\<2\>/1/g' -e 's/\<3\>/1/g' -e 's/\<5\>/4/g' -e 's/\<6\>/4/g' -e 's/\<8\>/7/g' -e 's/\<9\>/7/g' -e 's/\<11\>/10/g' -e 's/\<12\>/10/g' -e 's/\<14\>/13/g' -e 's/\<15\>/13/g' | sed -e 's/\<4\>/2/g' | sed -e 's/\<7\>/3/g' | sed -e 's/\<10\>/4/g' | sed -e 's/\<13\>/5/g' | sed -e 's/0//g' > rlnBeamTiltClass.tmp
